...
use catan::game::{legal, Phase, TurnPhase, DevelopmentPhase, Action, Error, Notification};
...
impl PythonPlayer {
    pub const ACTION_EXIT_SIGNAL: u16 = u16::MAX;
    pub const ACTION_RESET_SIGNAL: u16 = u16::MAX - 1;
    pub(crate) fn new<'a>(
        id: u8,
        format: PyObservationFormat,
        action_receiver: Receiver<u16>,
    ...
    fn make_legal_actions(&mut self, phase: &Phase, state: &State) -> (Array1<bool>, Array1<bool>) {
        // match phase {
        ...
        // }
        let mut normal_legal_actions = Array1::from_elem(self.normal_action_length + 1, false); // +1 for Trade action
        ...
        for (i, action) in self.possible_actions.iter().enumerate() {
            if legal::legal(phase, state, *action, trade_allowed).is_ok() {
        ...
        if trade_allowed && state.get_player_hand(self.position).resources.total() > 0 {
        ...
        (normal_legal_actions, trade_legal_actions)
    }
...
impl CatanPlayer for PythonPlayer {
    fn new_game(&mut self, position: PlayerId, state: &State) {
        ...
        generate_possible_actions(&mut possible_action_vec, self.position, state, true, true);
        self.possible_actions = possible_action_vec.into_iter().collect();

        let mut possible_action_without_trade_vec = Vec::new();
        generate_possible_actions(&mut possible_action_without_trade_vec, self.position, state, false, true);
        let nontrade_length = possible_action_without_trade_vec.len();
        self.normal_action_length = nontrade_length;
        self.trade_length = self.possible_actions.len() - nontrade_length;
        self.trade_count = 0;
    }

    fn pick_action(&mut self, phase: &Phase, state: &State) -> Action {
        let (normal_legal_actions, trade_legal_actions) = self.make_legal_actions(phase, state);
        let trade_allowed = self.trade_activated && self.trade_count < self.trade_limit;
        self.observation_sender.send(
            Some((
                self.id,
                match state.as_any().downcast_ref::<PythonState>() {
                    Some(python_state) => PyCatanObservation::new_python_array(self.format, self.position, python_state, state, phase, normal_legal_actions, trade_legal_actions, trade_allowed),
                    None => PyCatanObservation::new_array(self.format, self.position, state, phase, normal_legal_actions, trade_legal_actions, trade_allowed),
                }
            ))
        ).expect("Failed sending observation");
        thread::park();
        let action_index = self.action_receiver.recv().expect("Failed receiving action");
        match action_index {
            Self::ACTION_EXIT_SIGNAL => Action::Exit,
            Self::ACTION_RESET_SIGNAL => Action::Reset,
            _ => self.possible_actions[action_index as usize], // ここは trade アクションも含むものを使う
        }
    }
    ...
    fn notify(&mut self, notification: &Notification) {// とりあえず Trade の成功と失敗だけ通知で管理
        match notification {
            Notification::TradeAccepted => {
                self.trade_count += 1; // トレード成功したらトレード回数をカウント
            },
            Notification::TradeDeclined => {
                self.trade_count = self.trade_limit; // トレード拒否されたらそのターンはもうトレードできない
            },
            Notification::ResourcesRolled { .. } => {
                self.trade_count = 0; // 新しいターンが始まったらトレード回数リセット
            },
            _ => (),
        }
    }
